...
var recursiveErrorPropagation = function(self, res, e) {
    const { fnArray } = self;
    ...
    res = res || { value: self.__data, done: false };
    ...
    return function* () {
        ...
        if (utils.isError(self.__data)) {
            try {
                ...
                res = recursiveErrorPropagation(self, res, e);
            }
        } else {
            try {
        ...
        switch (type) {
            ...
            case 'task':
                self.busy = true;
                var p = promiseResolver(taskResolver(data, self, { threading: false })); // Run task in main Chip
                p.then(function (d) {
                    self.__data = d;
                    self.busy = false;
                });
            ...
            default:
                if (type.startsWith('array_')) {
                    ...
                    switch (arrayType) {
                        ...
                        case 'generatorFn':
                            self.busy = true;
                            p = promiseResolver(threadify(data, self, arrayType));
                            p.then((d) => {
                                self.__data = d;
                                self.busy = false;
                            });
                        ...
                        case 'task':
                            ...
                            return yield p;
                        case 'promise':
                            p = promiseResolver(Promise.all(data));
                            p.then(function (d) {
                                self.__data = d;
                                self.busy = false;
                            });
...
const checkProcessForDeadlock = (p) => {
    try {
        if (p.__coexec__process__ === true) {
            if (p.chip.pooling === true) {
                p.executioner.moveProcessToSubChip(p);
            }
        }    
    } catch (e) {
...
const promiseResolver = (p) => {
    checkProcessForDeadlock(p);
    return new Promise((resolve) => {
        return p.then(resolve).catch((error) => {
            type = utils.getType(error);
...
const threadify = (tasks, process, arrayType) => {
    ...
    switch (arrayType) {
        case 'generator':
            tasks = tasks.map((g, i) => new Task({ name: `t${i + 1}/${count}`, retries: 0 }, function* () { yield g; }));
            break;
        case 'generatorFn':
