        ...
        switch (type) {
            ...
            default:
                if (type.startsWith('array_')) {
                    ...
                    type = type.substr('array_'.length);
                    switch (type) {
                        case 'generator':
                            self.busy = true;
                        ...
                        case 'promise':
                            ...
                            return yield p;
                        default:
                            return yield new Error(`Unhandled nested thread command type: ${type}`);
                    }
                } else {
                    return yield new Error(`Unhandled thread command type: ${type}`);
                }
        }
    };
};
...
const threadify = (tasks, process, isGenerator = true) => {
    ...
    if (isGenerator === true) {
        ...
        tasks = tasks.map((g, i) => new Task(`${i}/${count}`, function* () { return g; }));
    }
